...
import { CreateTaskInput, Priority, Status, Task } from '@/types';

interface Props {
  open: boolean;
  onClose: () => void;
  onSubmit: (value: Omit<CreateTaskInput, 'id'> & { id?: string }) => void;
...
export default function TaskForm({ open, onClose, onSubmit, existingTitles, initial }: Props) {
  ...
  const handleSubmit = () => {
    ...
    const payload: Omit<CreateTaskInput, 'id'> & { id?: string } = {
      title: title.trim(),
